...
if __name__ == '__main__':
    ...
    naslovi = []
    for item in data:
        tekstovi.append(item[0])
        naslovi.append(item[1])

    df = calculate_document_frequencies(tekstovi)
    n = int(len(tekstovi))
    zborovi = get_vocabulary(tekstovi)

    tf_idf1 = process_document(test_doc, df, n, zborovi)
